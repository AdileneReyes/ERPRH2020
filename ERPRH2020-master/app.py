...
@app.route('/CancelarDep')
def CancelarDepartamento():
    return render_template('Departamentos/ConsultaGeneralDepartamentos.html')
...
def insertarDepartamento():
  #  id=request.form['id']
    ...
    cursor = conn.cursor()
    cursor.execute(
        'INSERT INTO RH.Departamentos (nombre) VALUES (?);', (nombre))
    conn.commit()
    return render_template('Departamentos/ConsultaGeneralDepartamentos.html')
...
# Lista General de Departamentos
@app.route('/listarDepartamentos')
def listarDepartamentos():
    cursor = conn.cursor()
    cursor.execute('Select idDepartamento, nombre from RH.Departamentos')
    data = cursor.fetchall()
    return render_template('Departamentos/ConsultaGeneralDepartamentos.html', departamenos=data)
...
def seleccionDepartamento():
    ...
    cursor = conn.cursor()
    cursor.execute(
        'Select idDepartamento, nombre from RH.Departamentos where idDepartamento={0}'.format(codigo))
    data = cursor.fetchall()
...
def eliminarDepartamento(id):
    ...
    conn.commit()
    return render_template('Departamentos/ConsultaGeneralDepartamento.html')
...
def editarDepartamento(id):
    cursor = conn.cursor()
    cursor.execute(
        'Select idDepartamento, nombre from RH.Departamentos where idDepartamento={0}'.format(id))
    data = cursor.fetchall()
    return render_template('Departamentos/EditarDepartamentos.html', departamentos=data[0])
...
def actualizarDepartamento(id):
   # idc = request.form['id']
    ...
    cursor = conn.cursor()
    cursor.execute(
        'Update RH.Departamentos set nombre=? where idDepartamento=?;'
         , (nombre, id))
    conn.commit()
    return render_template('Departamentos/ConsultaGeneralDepartamentos.html')
